...
func performanceTest(roleId int64, req proto.Message) {
	...
	//aresTcpClient.GetInnerClient(aresTcpClient.ROUTER).SendInnerMsg(protoGen.ProtoCode_PERFORMANCE_TEST_REQ, 0, req)
}

func performanceTestResFromWorld(roleId int64, res proto.Message) {
	//testRes := res.(*protoGen.PerformanceTestRes)
	//aresTcpClient.GetInnerClient(aresTcpClient.GATE_WAY).SendInnerMsg(protoGen.ProtoCode_PERFORMANCE_TEST_RES) 0, testRes)
}

func directToGame(roleId int64, req proto.Message) {
	echoMsg := req.(*protoGen.EchoReq)
	log.Infof("--------- directToGame  body =%s", echoMsg)
	aresTcpClient.GetInnerClient(global.GATE_WAY).SendMsg(protoGen.ProtoCode_DIRECT_FROM_GAME_CLIENT, roleId, echoMsg)
}
