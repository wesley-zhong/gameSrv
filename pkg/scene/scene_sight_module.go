...
// ISceneSightModule 场景视距管理器接口
type ISceneSightModule interface {
	// Init 初始化组件
	Init(beginPos, sceneSize *math.Vector2) error
	// PlaceEntity entity进入场景
	PlaceEntity(act IEntity) []IEntity
	// RemoveEntity entity离开场景
	RemoveEntity(act IEntity)
	// EntityMoveTo entity在场景中移动
	EntityMoveTo(act IEntity, destPos *math.Vector3) *EntityMoveToRet
	// EntityChangePhasing entity在场景中切换位面
	EntityChangePhasing(act IEntity, newPhasingId int64) *EntityMoveToRet
	// VisitGridsInSight 查询所有视距级别，entity附近满足visitor条件的entity集合
	VisitGridsInSight(act IEntity, visitor IVisitor)
	// IsEntityMoveGrid entity是否移动格子
	IsEntityMoveGrid(act IEntity, pos *math.Vector3) bool
	// FindPossibleRegionSet 寻找pos关联的region列表
	FindPossibleRegionSet(pos *math.Vector3) []IEntity
	// PosToCoordinate 获取坐标所在视野层的格子位置
	PosToCoordinate(rangeType *VisionLevelEnum, posX, posY int32) *Coordinate
	// PosToGridX 获取坐标所在视野层的X格位置
	PosToGridX(rangeType *VisionLevelEnum, posX int32) int32
	// PosToGridY 获取坐标所在视野层的Y格位置
	PosToGridY(rangeType *VisionLevelEnum, posY int32) int32
	// IsSameGrid 判断两个坐标在指定视野层上所在格子是否相等
	IsSameGrid(rangeType *VisionLevelEnum, posX1, posY1, posX2, posY2 int32) bool
	// CoordinateToPos 计算视野格的左上角的起始坐标
	CoordinateToPos(rangeType *VisionLevelEnum, gridX, gridY int32) *math.Vector2
	// InSceneMap 判断坐标是否在地图内
	InSceneMap(posX, posY int32) bool
	// GetScene 获取场景
	GetScene() IScene
	// GetBeginPos 获取开始位置
	GetBeginPos() *math.Vector2
	// GetEndPos 获取结束位置
	GetEndPos() *math.Vector2
	// GetSceneSize 获取场景大小
	GetSceneSize() *math.Vector2
	// String 返回字符串表示
	String() string
}
...
type EntityMoveToRet struct {
	MissEntities []IEntity
	MeetEntities []IEntity
}

// NewEntityMoveToRet 创建新的实体移动返回结果
func NewEntityMoveToRet(missEntities, meetEntities []IEntity) *EntityMoveToRet {
...
func NewSceneSightModule(scn IScene) *SceneSightModule {
...
func (s *SceneSightModule) PlaceEntity(act IEntity) []IEntity {
	return nil
}

// RemoveEntity entity离开场景 (抽象方法，子类实现)
func (s *SceneSightModule) RemoveEntity(act IEntity) {
}

// EntityMoveTo entity在场景中移动 (抽象方法，子类实现)
func (s *SceneSightModule) EntityMoveTo(act IEntity, destPos *math.Vector3) *EntityMoveToRet {
	return nil
}

// EntityChangePhasing entity在场景中切换位面 (抽象方法，子类实现)
func (s *SceneSightModule) EntityChangePhasing(act IEntity, newPhasingId int64) *EntityMoveToRet {
	return nil
}

// VisitGridsInSight 查询所有视距级别，entity附近满足visitor条件的entity集合 (抽象方法，子类实现)
func (s *SceneSightModule) VisitGridsInSight(act IEntity, visitor IVisitor) {
}

// IsEntityMoveGrid entity是否移动格子 (抽象方法，子类实现)
func (s *SceneSightModule) IsEntityMoveGrid(act IEntity, pos *math.Vector3) bool {
	return false
}

// FindPossibleRegionSet 寻找pos关联的region列表 (抽象方法，子类实现)
func (s *SceneSightModule) FindPossibleRegionSet(pos *math.Vector3) []IEntity {
	return nil
}

// PosToCoordinate 获取坐标所在视野层的格子位置
func (s *SceneSightModule) PosToCoordinate(rangeType *VisionLevelEnum, posX, posY int32) *Coordinate {
	gridX := (posX - s.BeginPos.X) / rangeType.GridWidth // 坐标要减去起始点的位置
	gridY := (posY - s.BeginPos.Y) / rangeType.GridWidth // 坐标要减去起始点的位置
	return NewCoordinate(gridX, gridY)
}

// PosToGridX 获取坐标所在视野层的X格位置
func (s *SceneSightModule) PosToGridX(rangeType *VisionLevelEnum, posX int32) int32 {
	return (posX - s.BeginPos.X) / rangeType.GridWidth // 坐标要减去起始点的位置
}

// PosToGridY 获取坐标所在视野层的Y格位置
func (s *SceneSightModule) PosToGridY(rangeType *VisionLevelEnum, posY int32) int32 {
	return (posY - s.BeginPos.Y) / rangeType.GridWidth // 坐标要减去起始点的位置
}

// IsSameGrid 判断两个坐标在指定视野层上所在格子是否相等
func (s *SceneSightModule) IsSameGrid(rangeType *VisionLevelEnum, posX1, posY1, posX2, posY2 int32) bool {
...
// CoordinateToPos 计算视野格的左上角的起始坐标
func (s *SceneSightModule) CoordinateToPos(rangeType *VisionLevelEnum, gridX, gridY int32) *math.Vector2 {
	// 获取视野格的宽度
	gridWidth := rangeType.GridWidth
...
// GetScene 获取场景
func (s *SceneSightModule) GetScene() IScene {
	return s.Scene
}
