...
func NewGridMgr(scn IScene, rangeType *VisionLevelEnum) *GridMgr {
	return &GridMgr{
		scene:     scn,
		rangeType: rangeType,
	}
...
// Init 初始化
func (g *GridMgr) Init(length, width int) error {
	g.length = length
	g.width = width
	mesh, err := NewMesh(length, width)
	if err != nil {
		return err
	}
	g.mesh = mesh
...
// PlaceEntity 放置实体
func (g *GridMgr) PlaceEntity(actor IEntity, gridX, gridY int) {
	grid := g.GetGrid(gridX, gridY)
	grid.AddEntity(actor)
}

// PlaceRegionEntity 放置区域实体
func (g *GridMgr) PlaceRegionEntity(region IEntity, coords []*Coordinate) {
	for _, coord := range coords {
		grid := g.GetGrid(int(coord.X), int(coord.Y))
		if grid == nil {
			log.Printf("placeRegionEntity getGrid doesn't exist! coord = %v, grid_mgr = %v",
				coord, g)
			continue
		}
	...
	for _, playerViewMgr := range g.scene.GetPlayerViewMgrMap() {
		...
		// 检查AvatarTeam是否在区域内
		// 需要Region类型断言来调用IsInRegion方法
		if regionEntity, ok := region.(interface {
			IsInRegion(*math.Vector3) bool
			AddEntity(IEntity, bool)
		}); ok {
			if regionEntity.IsInRegion(avatarTeam.GetLocation().GetPosition()) {
				regionEntity.AddEntity(avatarTeam, true)
			}
		}
	}
}
...
// RemoveEntity 删除实体
func (g *GridMgr) RemoveEntity(actor IEntity) {
	// 获取实体所在的网格
	grid := actor.GetGrid()
	if grid == nil {
...
func (g *GridMgr) RemoveRegionEntity(region IEntity, coords []*Coordinate) {
	for _, coord := range coords {
		grid := g.GetGrid(int(coord.X), int(coord.Y))
		if grid == nil {
			continue
	...
	// 清除区域内所有实体
	// 需要Region类型断言来调用DelAllEntity方法
	if regionEntity, ok := region.(interface{ DelAllEntity(bool) }); ok {
		regionEntity.DelAllEntity(true)
	}
}

// EntityMoveTo 实体移动
func (g *GridMgr) EntityMoveTo(actor IEntity, gridX, gridY int) bool {
	// 获取实体当前所在网格
	curGrid := actor.GetGrid()
	if curGrid == nil {
		log.Printf("entityMoveTo curGrid doesn't exist! Entity = %v", actor)
		return false
	}

	destGrid := g.GetGrid(gridX, gridY)
	if destGrid == nil {
		log.Printf("entityMoveTo getGrid doesn't exist! Entity = %v, posX = %v, posY = %v", actor, gridX, gridY)
		return false
	}
...
func (g *GridMgr) VisitGridsInSight(gridX, gridY int, visitor IVisitor) {
...
func (g *GridMgr) VisitDiffGrids(fromGridX, fromGridY, toGridX, toGridY int, miss, add IVisitor) {
...
// visitGrid 访问指定的格子
func (g *GridMgr) visitGrid(gridX, gridY int, visitor IVisitor) {
...
func (g *GridMgr) String() string {
	return fmt.Sprintf("[scene_id:%s,range_type:%s,length:%d,width:%d,sightRadius:%d]",
		g.scene.String(), g.rangeType.String(), g.length, g.width, g.rangeType.GetSightRadius())
}
