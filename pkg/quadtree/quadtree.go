...
// TotalNodes - Retrieve the total number of sub-Quadtree[T]s in a Quadtree[T]
...
func (qt *Quadtree[T]) split() {
	...
	//top right node (0)
	qt.Nodes = append(qt.Nodes, Quadtree[T]{
		Bounds: Bounds{
			X:      x + subWidth,
			Y:      y,
			Width:  subWidth,
		...
		Level:      nextLevel,
		Objects:    make([]Entity[T], 0),
		Nodes:      make([]Quadtree[T], 0, 4),
	})
...
func (qt *Quadtree[T]) Insert(pRect Entity[T]) {
	...
	// If we have subnodes within the Quadtree[T]
	...
	// If we don't subnodes within the Quadtree[T]
...
// RetrieveIntersections - Bring back all the bounds in a Quadtree[T] that intersect with a provided bounds
func (qt *Quadtree[T]) RetrieveIntersections(find Bounds) []Entity[T] {

	var foundIntersections []Entity[T]
