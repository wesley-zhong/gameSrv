...
func performanceTest(pid int64, req proto.Message) {
	...
	//client.getInnerClient(client.GAME).SendInnerMsg(protoGen.MsgId_PERFORMANCE_TEST_RES, 0, res)
}

func onDirectToWorld(pid int64, req proto.Message) {
	echoMsg := req.(*protoGen.EchoReq)
	client.SendToGameServer(pid, protoGen.MsgId_DIRECT_FROM_WORLD_CLIENT, echoMsg)
